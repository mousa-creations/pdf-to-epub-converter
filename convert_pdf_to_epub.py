...
def check_dependencies():
    ...
    try:
        ...
        print(
            "   For scanned PDFs, install it with: brew install tesseract tesseract-lang"
        )
        ...
        try:
            ...
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if text_content and has_garbled_text(text_content):
                    print(
                        "   ⚠️  Detected garbled text (custom font encoding) - will use OCR"
                    )
                    return True
...
def extract_with_ocr(pdf_path):
    ...
    Enhanced with higher DPI, image sharpening, and contrast improvement.
    ...
    try:
        ...
        # Increased DPI from 300 to 400 for better quality
        images = convert_from_path(pdf_path, dpi=400)

        print(f"🔍 Performing OCR on {len(images)} pages (this may take a while)...")
        extracted_texts = []

        # Better Tesseract config for Arabic
        ...
        for i, image in enumerate(images, 1):
            ...
            # Step 4: OCR with improved config
...
def extract_text_directly(pdf_path):
    ...
    try:
        ...
        print(
            "📖 Extracting text from PDF (using pdfplumber for better Arabic support)..."
        )
        extracted_texts = []
        ...
        print(
            "   Note: For better Arabic support, install pdfplumber: pip install pdfplumber"
        )
    except Exception as e:
    ...
    try:
        ...
        for i, page_text in enumerate(text_pages, 1):
            ...
            chapter = epub.EpubHtml(
                title=f"Page {i}", file_name=f"{chapter_id}.xhtml", lang="ar"
            )
    ...
    if FORCE_OCR:
        ...
        if is_scanned:
            ...
            if has_garbled_text(sample_text):
                print()
                print(
                    "⚠️  Warning: Extracted text appears garbled (custom font encoding detected)"
                )
